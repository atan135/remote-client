...
- 浏览器登录态保存在 `user_sessions` 表中，通过 HTTP-only cookie 维持
...
- 如果设备 A 已登录，同账号在设备 B 再登录时，设备 A 不会被自动挤下线
- 设备 A、B 后续都会继续收到各自浏览器连接上的实时更新
...
9. 在文件预览面板打开一个文本文件：绝对路径可直接填写；相对路径可填写基准目录，也可在活动终端会话中按当前终端目录解析；唯一模糊匹配的文件名应能打开。
...
7. 完成一次安全命令下发
8. 完成一次远程终端创建和交互
9. 完成一次远程文本文件预览：绝对路径应可直接读取；相对路径可通过明确基准目录读取，也可通过活动终端当前目录读取；唯一模糊文件名应可打开。
