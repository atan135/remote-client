...
这样平台可以按设备维护明确的用户归属绑定，并保证同一设备不会被多个用户重复占用。

补充约束：

- 同一个 `agentId` 在平台内只允许存在一条有效 `auth_code` 绑定
- 如果设备已经被用户 A 绑定，用户 B 不能再为同一 `agentId` 新建绑定
- 因此 `auth_code` 语义从“每用户-每设备一条”收紧为“每设备全局唯一归属”
...
2. 登录 Web 控制台
3. 若该 `agentId` 尚未被其他用户占用，则为其创建或更新唯一 `auth_code`
4. 后续只有当前绑定用户才能对该设备发起安全命令、终端会话和文件读取

若设备已被其他用户绑定：

- 普通用户创建时应直接失败
- 需要当前绑定用户主动解绑，或由管理员执行绑定转移
