...
- 交互式终端会话
- 远程文本文件预览
- 用户/会话/用户审核/设备审核/`auth_code` 管理
- 基于 Web 控制台的设备视图
- Web 对话页里的普通命令确认卡和 Codex / AI Agent 终端会话模式
...
虽然当前 `remote-client` 已有 Web 对话页和 Codex 会话模式，但它仍是“浏览器控制台 -> 指定目标机”的远控入口，不是飞书、Telegram、Slack、微信这类多聊天平台桥接系统。
...
- 前端已经拆出 router、Pinia store、页面组件和通用组件，但整体产品化能力仍偏早期
