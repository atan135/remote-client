...
`ChatPage.vue` 复用 `useConsoleStore()`，把设备、命令、终端会话和远程文件读取入口传给 `ChatTab.vue`。

文件弹窗的保存状态和保存动作不通过 props 注入；`ChatTab.vue` 内部直接复用 `useConsoleStore()` 的 `isSavingRemoteFileForContext()`、`getRemoteFileSaveErrorForContext()` 和 `saveRemoteFile()`。
...
- 如果最终结果像文件路径，聊天页展示文件卡片，并用 `/api/remote-files/read` 打开远程文本文件；预览弹窗保存时复用 `/api/remote-files/write`。
...
文件内容读取仍走 `/api/remote-files/read` 与 `file.read.secure`，保存走 `/api/remote-files/write` 与 `file.write.secure`，由 `localapp` 使用 `fs/stat/read/write` 完成，不通过终端命令读写文件。Windows 目标机上的 `localapp` 不会自动转换 `/c/...` 这类 Git Bash / POSIX 风格路径。

聊天页从 Codex 文件卡片打开的预览弹窗可以直接编辑和保存远程文本文件，底层复用 `store.saveRemoteFile()`，而不是把保存转换成终端命令：

- `.txt` 和其他普通文本默认进入纯文本编辑。
- `.md`、`.markdown`、`.mdown`、`.mkd`、`.mkdn` 默认 Markdown 预览，可切换纯文本查看或编辑，保存时写回纯文本。
- 截断读取、空 `resolvedPath`、读取失败、二进制文件、路径解析冲突、远程文件已变化冲突都不能保存。
- 保存请求带上最近一次成功读取得到的 `resolvedPath`、`modifiedAt`、`totalBytes`，由 agent 在写入前校验 `mtime` 和 `size`；冲突时提示用户重新打开文件。
...
- 文件保存必须走 `/api/remote-files/write` 和 `file.write.secure`，不能通过终端命令写文件。
- 不在前端、日志或接口响应中暴露私钥。
- 不在日志、接口错误或 UI 通知中输出完整大段文件内容。
...
9. Codex 文件卡片打开 `.md` 时默认 Markdown 预览，切换编辑保存后重新打开内容一致。
10. Codex 文件卡片打开 `.txt` 时默认可编辑，保存后目标机器文件内容变化。
11. 前端构建通过：`npm run build:client`。
