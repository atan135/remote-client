...
点击“打开文件”后，前端调用现有 `openRemoteFile()`。绝对路径可直接打开；相对路径必须带 `baseCwd`，聊天页会优先使用会话创建时记录的 `cwd` 作为基准目录，并可把当前 Codex 终端会话的 `sessionId` 作为可选上下文一并发送。

服务端不会实时探测 Codex 终端当前目录，也不会让文件读取自动跟随当前终端目录。Codex 返回相对路径时，需要聊天页持有会话创建 `cwd` 或显式 `baseCwd`；如果无法确定基准目录，Codex 应返回绝对路径。Windows 目标机上的 `localapp` 不会自动转换 `/c/...` 这类 Git Bash / POSIX 风格路径。
...
8. Codex 返回绝对路径时能直接打开远程文件预览；返回相对路径时，只有存在会话创建 `cwd` 或显式 `baseCwd` 才能解析。
