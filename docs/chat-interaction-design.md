...
远程文件保存处于协议设计阶段。聊天页后续如果从文件卡片或预览弹窗进入编辑保存，应复用远程文件保存协议，而不是把保存转换成终端命令：

- `.txt` 可直接进入纯文本编辑。
- `.md`、`.markdown`、`.mdown`、`.mkd`、`.mkdn` 可在 Markdown 预览和纯文本编辑之间切换，保存时写回纯文本。
- 截断读取、空 `resolvedPath`、读取失败、二进制文件、路径解析冲突、远程文件已变化冲突都不能保存。
- 保存请求应带上最近一次成功读取得到的 `resolvedPath`、`modifiedAt`、`totalBytes`，由 agent 在写入前校验 `mtime` 和 `size`；冲突时提示用户重新打开文件。
...
- 文件保存必须走后续的 `/api/remote-files/write` 和 `file.write.secure`，不能通过终端命令写文件。
- 不在前端、日志或接口响应中暴露私钥。
