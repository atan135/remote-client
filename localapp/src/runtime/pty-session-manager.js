...
export class PtySessionManager {
  ...
  writeInput(sessionId, input) {
    ...
    logEvent(this.commandLogger, "info", "terminal.session.input", {
      ...
      ...createTerminalInputDiagnostics(input)
    });
...
function createTerminalInputDiagnostics(input) {
  ...
  return {
    ...
    endsWithLineBreak: /[\r\n]$/.test(text),
    commandKind: classifyTerminalInput(text),
    containsCodexExec: /\bcodex\s+exec\b/i.test(text),
    containsCodexAskForApproval: /--ask-for-approval\b/i.test(text),
    containsRemoteClientMarker: /\[remote-client\]\s+codex\.exec\./i.test(text),
    containsFinalStartMarker: /<<<FINAL>>>/.test(text),
    containsFinalEndMarker: /<<<END_FINAL>>>/.test(text)
  };
}
